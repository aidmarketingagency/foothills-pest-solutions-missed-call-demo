(function(){
  ...
  // -- SMS thread staged reveal: IntersectionObserver, re-arms on scroll re-entry --
  ...
  function playThread(){
    ...
    clearTimers();
    resetThread();
    // 4-bubble sequence: caller, AI, caller, AI
    var seq = [
      ...
      { t: 2920, action: function(){ bubbles[2].classList.add('show'); } },
      { t: 3480, action: function(){ typers[2].classList.add('show'); } },
      { t: 4520, action: function(){ typers[2].classList.remove('show'); bubbles[3].classList.add('show'); playing = false; } }
    ];
  ...
  // Re-arms on scroll re-entry: observer stays alive for page lifetime
    ...
    var demoIO = new IntersectionObserver(function(entries){
      entries.forEach(function(e){
        if (e.isIntersecting){
          ...
          // left viewport: cancel in-flight sequence so re-entry starts clean
          clearTimers();
          playing = false;
          resetThread();
        }
  ...
  // -- Reveal-on-scroll for sections (plays once) --
  ...
  // -- Animated $650 counter: counts up from 0, re-arms on scroll re-entry --
  // Stat source: https://www.angi.com/articles/how-much-does-termite-treatment-cost.htm
  var STAT_TARGET = 650;
  ...
  function runCount(){
    ...
    var start = null;
    function step(ts){
      if (runId !== countRun) return; // superseded -- stop
      if (!start) start = ts;
  ...
  if (stickyCta && ctaPanel && 'IntersectionObserver' in window){
    ...
    var ctaIO = new IntersectionObserver(function(entries){
      entries.forEach(function(e){
        if (e.isIntersecting){ stickyCta.classList.add('hide'); }
        else { stickyCta.classList.remove('hide'); }
      });
    }, { threshold: 0.2 });
